...
export default function OperativeCard(props) {
    ...
    const renderWeapon = (weapon) => {
        if (weapon.weaponprofiles.length > 1) {
            return (
                <>
                    {weapon.weaponprofiles.length > 1 && <Table.Tr key={weapon.wepid}>
                        <Table.Td>
                    ...
                    {weapon.weaponprofiles.map((profile) => (
                ...
                <Table.Tr key={weapon.wepid}>
                    <Table.Td>
                        <Group wrap="nowrap" gap="sm">
                            {weapon.weptype === "M" ?
                                <IconSwords size={20} stroke={1.5} /> : <IconCrosshair size={20} stroke={1.5} />}
                            {weapon.wepname}{weapon.weaponprofiles[0].SR ? ` (${weapon.weaponprofiles[0].SR})` : ''}
                        </Group>
                    </Table.Td>
                    <Table.Td>{weapon.weaponprofiles[0].A}</Table.Td>
                    <Table.Td>{weapon.weaponprofiles[0].BS}</Table.Td>
                    <Table.Td>{weapon.weaponprofiles[0].D}</Table.Td>
                </Table.Tr>
            </>
        )
    }
